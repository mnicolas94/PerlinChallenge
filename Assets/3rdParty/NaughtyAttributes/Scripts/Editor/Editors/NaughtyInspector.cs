...
namespace NaughtyAttributes.Editor
{
	...
	public class NaughtyInspector : UnityEditor.Editor
	{
		...
		private void OnEnable()
		{
			...
			_fields = ReflectionUtility.GetAllFields(target, f => serializedObject.FindProperty(f.Name) != null);

			// If there are no NaughtyAttributes use default inspector
			if (_fields.All(f => f.GetCustomAttributes(typeof(NaughtyAttribute), true).Length == 0))
			{
			...
			_nonSerializedFields = ReflectionUtility.GetAllFields(
				target, f => f.GetCustomAttributes(typeof(DrawerAttribute), true).Length > 0 && serializedObject.FindProperty(f.Name) == null);
